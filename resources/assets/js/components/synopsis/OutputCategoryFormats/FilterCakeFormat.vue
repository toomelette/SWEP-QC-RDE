<template>
    <div v-if="collection.length > 0">
        ...
        <h3 style="text-align:center;">TABLE XII.  FILTER CAKE</h3>
        <br>
                ...
                <template v-for="(region, region_key) in regions">
                    <template v-if="filterCollection(collection, region_key).length > 0">
                        <tr>
                            ...
                            <td style="font-weight:bold;">{{ utilTotalPerRegion(collection, region_key, "filtered_juice", 2) }}</td>
                            <td style="font-weight:bold;">{{ utilTotalPerRegion(collection, region_key, "purity_drop", 2) }}</td>
                        </tr>
                        <tr v-for="(data, data_key) in filterCollection(collection, region_key)">
                            ...
                            <td id="mid-vert">{{ data.filtered_juice }}</td>
                            <td id="mid-vert">{{ data.purity_drop }}</td>
                        </tr>
                    </template>
                </template>
                <tr>
                    ...
                    <td style="font-weight:bold;">{{ utilTotalPerCY(collection, "filtered_juice", 2) }}</td>
                    <td style="font-weight:bold;">{{ utilTotalPerCY(collection, "purity_drop", 2) }}</td>
                </tr>
            </tbody>
        </table>
        </div>
                    ...
                    Object.values(collection).forEach(data => {
                        if(data.mill.report_region == region){
                            array.push({
                                ...
                                filtered_juice: this.utilFilterFloat(data.filtered_juice, 2),
                                purity_drop: this.utilFilterFloat(data.purity_drop, 2),
                            });
                        }
                    });
                }
