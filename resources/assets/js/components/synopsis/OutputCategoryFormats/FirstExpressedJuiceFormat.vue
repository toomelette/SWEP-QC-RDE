<template>
    <div v-if="collection.length > 0">
        ...
        <h3 style="text-align:center;">TABLE VI.  FIRST EXPRESSED JUICE</h3>
        <br>
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th style="text-align:center;">DEREASE <br>IN AP</th>
                </tr>    
            </thead>    
            <tbody>
                <template v-for="(region, region_key) in regions">
                    <template v-if="filterCollection(collection, region_key).length > 0">
                        <tr>
                            ...
                            <td style="font-weight:bold;">{{ utilTotalPerRegion(collection, region_key, "decrease_in_ap", 2) }}</td>
                        </tr>
                        <tr v-for="(data, data_key) in filterCollection(collection, region_key)">
                            ...
                            <td id="mid-vert">{{ data.decrease_in_ap }}</td>
                        </tr>
                    </template>
                </template>
                <tr>
                    ...
                    <td style="font-weight:bold;">{{ utilTotalPerCY(collection, "decrease_in_ap", 2) }}</td>
                </tr>
            </tbody>
        </table>
        </div>
                    ...
                    Object.values(collection).forEach(data => {
                        if(data.mill.report_region == region){
                            array.push({
                                ...
                                decrease_in_ap: this.utilFilterFloat(data.decrease_in_ap, 2), 
                            });
                        }
                    });
                }
