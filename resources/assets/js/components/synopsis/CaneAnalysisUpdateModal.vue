...
<script>
    ...
    export default { 
        ...
        created() {
            ...
            EventBus.$on('OPEN_CANE_ANALYSIS_UPDATE_MODAL', (data) => {
                this.showModal(data);
            });
        ...
        methods: {
            ...
            showModal(data){ 
                ...
                axios.get('synopsis/cane_analysis/' + data.update_key)
                    .then((response) => { 
                        if(response.status == 200){
                            let cane_analysis = response.data.data;
                            this.update_key = cane_analysis.slug;
                            this.crop_year_id =  { code:cane_analysis.crop_year.crop_year_id, label:cane_analysis.crop_year.name };
                            this.mill_id = { code:cane_analysis.mill.mill_id, label:cane_analysis.mill.name };
                            this.percent_pol =  this.utilCheckFloat(cane_analysis.percent_pol);
                            this.percent_sucrose =  this.utilCheckFloat(cane_analysis.percent_sucrose);
                            this.percent_fiber =  this.utilCheckFloat(cane_analysis.percent_fiber);
                            this.percent_trash =  this.utilCheckFloat(cane_analysis.percent_trash); 
                            this.pol_percent_fiber =  this.utilCheckFloat(cane_analysis.pol_percent_fiber);
                        }
                    }); 
            ...
            update(){ 
                ...
                axios.post('synopsis/cane_analysis/' + this.update_key, {
                    ...
                    percent_trash: this.utilCheckFloat(this.percent_trash), 
                    pol_percent_fiber: this.utilCheckFloat(this.pol_percent_fiber), 
                })
                .then((response) => {
