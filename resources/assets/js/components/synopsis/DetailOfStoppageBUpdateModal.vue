...
<script>
    ...
    export default { 
        ...
        created() {
            ...
            EventBus.$on('OPEN_DETAIL_OF_STOPPAGE_B_UPDATE_MODAL', (data) => {
                this.showModal(data);
            });
        ...
        methods: {
            ...
            showModal(data){ 
                ...
                axios.get('synopsis/detail_of_stoppage_b/' + data.update_key)
                    .then((response) => { 
                        if(response.status == 200){
                            let detail_of_stoppage_b = response.data.data;
                            this.update_key = detail_of_stoppage_b.slug;
                            this.crop_year_id =  { code:detail_of_stoppage_b.crop_year.crop_year_id, label:detail_of_stoppage_b.crop_year.name };
                            this.mill_id = { code:detail_of_stoppage_b.mill.mill_id, label:detail_of_stoppage_b.mill.name };
                            this.due_cleaning_hrs =  this.utilCheckFloat(detail_of_stoppage_b.due_cleaning_hrs);
                            this.due_cleaning_tet =  this.utilCheckFloat(detail_of_stoppage_b.due_cleaning_tet);
                            this.due_no_unavoidable_hrs =  this.utilCheckFloat(detail_of_stoppage_b.due_no_unavoidable_hrs);
                            this.due_no_unavoidable_tet =  this.utilCheckFloat(detail_of_stoppage_b.due_no_unavoidable_tet);
                            this.due_miscellaneous_hrs =  this.utilCheckFloat(detail_of_stoppage_b.due_miscellaneous_hrs);
                            this.due_miscellaneous_tet =  this.utilCheckFloat(detail_of_stoppage_b.due_miscellaneous_tet);
                        }
                    }); 
            ...
            update(){ 
                ...
                axios.post('synopsis/detail_of_stoppage_b/' + this.update_key, {
                    crop_year_id: this.crop_year_id?.code.toString(), 
                    mill_id: this.mill_id?.code.toString(),
                    due_cleaning_hrs: this.utilCheckFloat(this.due_cleaning_hrs), 
                    due_cleaning_tet: this.utilCheckFloat(this.due_cleaning_tet), 
                    due_no_unavoidable_hrs: this.utilCheckFloat(this.due_no_unavoidable_hrs), 
                    due_no_unavoidable_tet: this.utilCheckFloat(this.due_no_unavoidable_tet), 
                    due_miscellaneous_hrs: this.utilCheckFloat(this.due_miscellaneous_hrs), 
                    due_miscellaneous_tet: this.utilCheckFloat(this.due_miscellaneous_tet), 
                })
                .then((response) => {
