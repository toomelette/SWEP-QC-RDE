...
<script>
    ...
    export default { 
        ...
        created() {
            ...
            EventBus.$on('OPEN_TEN_YR_AGRI_PARAM_UPDATE_MODAL', (data) => {
                this.showModal(data);
            });
        ...
        methods: {
            ...
            showModal(data){ 
                ...
                axios.get('synopsis/ten_yr_agri_param/' + data.update_key)
                    .then((response) => { 
                        if(response.status == 200){
                            let ten_yr_agri_param = response.data.data;
                            this.update_key = ten_yr_agri_param.slug;
                            this.crop_year_id =  { code:ten_yr_agri_param.crop_year.crop_year_id, label:ten_yr_agri_param.crop_year.name };
                            this.area_harvested =  this.utilCheckFloat(ten_yr_agri_param.area_harvested);
                            this.tc_ha =  this.utilCheckFloat(ten_yr_agri_param.tc_ha);
                            this.lkg_tc =  this.utilCheckFloat(ten_yr_agri_param.lkg_tc);
                            this.lkg_ha =  this.utilCheckFloat(ten_yr_agri_param.lkg_ha);
                        }
                    }); 
            ...
            update(){ 
                ...
                axios.post('synopsis/ten_yr_agri_param/' + this.update_key, {
                    crop_year_id: this.crop_year_id?.code.toString(), 
                    area_harvested: this.utilCheckFloat(this.area_harvested), 
                    tc_ha: this.utilCheckFloat(this.tc_ha), 
                    lkg_tc: this.utilCheckFloat(this.lkg_tc), 
                    lkg_ha: this.utilCheckFloat(this.lkg_ha),  
                })
                .then((response) => {
