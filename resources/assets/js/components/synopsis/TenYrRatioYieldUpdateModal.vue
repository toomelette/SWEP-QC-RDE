...
<script>
    ...
    export default { 
        ...
        created() {
            ...
            EventBus.$on('OPEN_TEN_YR_RATIO_YIELD_UPDATE_MODAL', (data) => {
                this.showModal(data);
            });
        ...
        methods: {
            ...
            showModal(data){ 
                ...
                axios.get('synopsis/ten_yr_ratio_yield/' + data.update_key)
                    .then((response) => { 
                        if(response.status == 200){
                            let ten_yr_ratio_yield = response.data.data;
                            this.update_key = ten_yr_ratio_yield.slug;
                            this.crop_year_id =  { code:ten_yr_ratio_yield.crop_year.crop_year_id, label:ten_yr_ratio_yield.crop_year.name };
                            this.imbibition_fiber_ratio =  this.utilCheckFloat(ten_yr_ratio_yield.imbibition_fiber_ratio);
                            this.rendement =  this.utilCheckFloat(ten_yr_ratio_yield.rendement);
                            this.quality_ratio =  this.utilCheckFloat(ten_yr_ratio_yield.quality_ratio);
                            this.kg_mollasses_per_ton_cane =  this.utilCheckFloat(ten_yr_ratio_yield.kg_mollasses_per_ton_cane);
                            this.kg_bagasse_per_ton_cane =  this.utilCheckFloat(ten_yr_ratio_yield.kg_bagasse_per_ton_cane);
                            this.kg_fc_per_ton_cane =  this.utilCheckFloat(ten_yr_ratio_yield.kg_fc_per_ton_cane);
                        }
                    }); 
            ...
            update(){ 
                ...
                axios.post('synopsis/ten_yr_ratio_yield/' + this.update_key, {
                    crop_year_id: this.crop_year_id?.code.toString(), 
                    imbibition_fiber_ratio: this.utilCheckFloat(this.imbibition_fiber_ratio), 
                    rendement: this.utilCheckFloat(this.rendement), 
                    quality_ratio: this.utilCheckFloat(this.quality_ratio), 
                    kg_mollasses_per_ton_cane: this.utilCheckFloat(this.kg_mollasses_per_ton_cane), 
                    kg_bagasse_per_ton_cane: this.utilCheckFloat(this.kg_bagasse_per_ton_cane), 
                    kg_fc_per_ton_cane: this.utilCheckFloat(this.kg_fc_per_ton_cane), 
                })
                .then((response) => {
