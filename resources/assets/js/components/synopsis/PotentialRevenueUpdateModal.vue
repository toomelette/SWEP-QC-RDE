...
<script>
    ...
    export default { 
        ...
        created() {
            ...
            EventBus.$on('OPEN_POTENTIAL_REVENUE_UPDATE_MODAL', (data) => {
                this.showModal(data);
            });
        ...
        methods: {
            ...
            showModal(data){ 
                ...
                axios.get('synopsis/potential_revenue/' + data.update_key)
                    .then((response) => { 
                        if(response.status == 200){
                            let potential_revenue = response.data.data;
                            this.update_key = potential_revenue.slug;
                            this.crop_year_id =  { code:potential_revenue.crop_year.crop_year_id, label:potential_revenue.crop_year.name };
                            this.mill_id = { code:potential_revenue.mill.mill_id, label:potential_revenue.mill.name };
                            this.due_bh =  this.utilCheckFloat(potential_revenue.due_bh);
                            this.due_trash =  this.utilCheckFloat(potential_revenue.due_trash);
                            this.total =  this.utilCheckFloat(potential_revenue.total);
                            this.potential_revenue =  this.utilCheckFloat(potential_revenue.potential_revenue); 
                        }
                    }); 
            ...
            update(){ 
                ...
                axios.post('synopsis/potential_revenue/' + this.update_key, {
                    crop_year_id: this.crop_year_id?.code.toString(), 
                    mill_id: this.mill_id?.code.toString(),
                    due_bh: this.utilCheckFloat(this.due_bh), 
                    due_trash: this.utilCheckFloat(this.due_trash), 
                    total: this.utilCheckFloat(this.total), 
                    potential_revenue: this.utilCheckFloat(this.potential_revenue), 
                })
                .then((response) => {
