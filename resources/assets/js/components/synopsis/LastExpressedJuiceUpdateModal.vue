...
<script>
    ...
    import EventBus from '../../SynLastExpressedJuiceMain';
    ...
    export default { 
        ...
        created() {
            ...
            EventBus.$on('OPEN_LAST_EXPRESSED_JUICE_UPDATE_MODAL', (data) => {
                this.showModal(data);
            });
        ...
        methods: {
            ...
            showModal(data){ 
                ...
                axios.get('synopsis/last_expressed_juice/' + data.update_key)
                    .then((response) => { 
                        if(response.status == 200){
                            let last_expressed_juice = response.data.data;
                            this.update_key = last_expressed_juice.slug;
                            this.crop_year_id =  { code:last_expressed_juice.crop_year.crop_year_id, label:last_expressed_juice.crop_year.name };
                            this.mill_id = { code:last_expressed_juice.mill.mill_id, label:last_expressed_juice.mill.name };
                            this.brix =  this.utilCheckFloat(last_expressed_juice.brix);
                            this.percent_pol =  this.utilCheckFloat(last_expressed_juice.percent_pol);
                            this.apparent_purity =  this.utilCheckFloat(last_expressed_juice.apparent_purity);
                        }
                    }); 
            ...
            update(){ 
                ...
                axios.post('synopsis/last_expressed_juice/' + this.update_key, {
                ...
                .then((response) => {
                    ...
                    if(response.status == 200){
                        ...
                        EventBus.$emit('LAST_EXPRESSED_JUICE_UPDATE_LIST', {'key': response.data.key});
